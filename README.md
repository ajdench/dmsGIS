...
- The first approved sidebar-prototype promotion slice is now live in production Labels: shared sidebar row/popover primitives live in `src/components/sidebar/`, while pane-specific label control definitions stay feature-owned in `src/features/labels/labelPanelFields.ts` and continue to drive `src/store/appStore.ts` basemap label state.
