...
**Notes:** This cleanup is now materially complete on `2026-04-02`. Phase 1 removed the orphaned pane files and their narrow tests. Phase 2 removed the orphaned `PmcPanel.tsx` / `OverlayPanel.tsx` files and the old `src/components/sidebar/` shell after detaching `ScenarioPlaygroundPane.tsx`. Phase 3 removed `src/components/sidebarReplacement/` and `src/styles/sidebarReplacement.css`. Phase 5 then removed the dead old-sidebar CSS/tokens from `src/styles/global.css` and gave `ScenarioPlaygroundPane.tsx` its own local shell/button class names. Remaining follow-up here, if wanted later, is historical-doc grooming rather than more working-app tree cleanup.
**Files likely touched:** `src/components/layout/ScenarioPlaygroundPane.tsx`, `src/styles/global.css`, current-truth docs, and the focused sidebar/playground test seam.
