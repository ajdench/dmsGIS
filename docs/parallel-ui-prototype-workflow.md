...
- parent-row and pane visibility should aggregate from immediate children only, but stay tri-state aware so mixed child sections propagate `Ox` upward rather than collapsing back to plain `On`/`Off`
