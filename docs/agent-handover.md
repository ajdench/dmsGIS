...
- the map pane now also carries a temporary square grey placeholder pane in the top-right corner, sized from the zoom-control parent pane width and aligned to the same outer margins, with a centered inert white square button matching the zoom-button size; the earlier crosshair toggle behavior is currently turned off
