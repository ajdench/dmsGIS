...
- that injected Royal Navy contribution row now resolves from the last content slot above the metrics stack in receiving Region cards, with its bottom offset tied to `3x` the internal metrics row-gap token used between the PAR lines below plus a direct `2px` downward optical nudge on the row itself
