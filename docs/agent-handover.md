...
- that injected Royal Navy contribution row now resolves from the last content slot above the metrics stack in receiving Region cards, with its bottom offset tied to `4x` the internal metrics row-gap token used between the PAR lines below
