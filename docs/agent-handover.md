...
- the `Regionalise` / `Unregionalise` control pill in the special `Royal Navy` card now uses that same middle-band contract, but its reserved top/bottom clearance is balanced evenly so the pill sits centered within its own band while the card still keeps extra surrounding clearance
