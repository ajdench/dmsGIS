...
- Facility point styling and point selection can now use draft-aware scenario region remapping through `src/features/map/scenarioFacilityMapping.ts`, and first derived facility count summaries now exist in `src/features/map/scenarioFacilityMetrics.ts`.
