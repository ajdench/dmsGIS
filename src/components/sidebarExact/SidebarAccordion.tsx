import * as Accordion from '@radix-ui/react-accordion';
import type { ButtonHTMLAttributes, ReactNode } from 'react';
import type { SidebarPillSummary } from '../../lib/sidebar/contracts';
import type { SidebarVisibilityState } from '../../lib/sidebar/visibilityTree';
import {
  ExactDragHandle,
  ExactMetricPill,
  ExactToggleButton,
} from './SidebarControls';

interface SidebarAccordionProps {
  value: string[];
  onValueChange: (value: string[]) => void;
  children: ReactNode;
  level?: 'pane' | 'subpane';
}

interface SidebarAccordionItemProps {
  id: string;
  title: string;
  subtitle?: string;
  badge?: SidebarPillSummary;
  children: ReactNode;
  level?: 'pane' | 'subpane';
  panel?: boolean;
  enabled?: boolean;
  toggleState?: SidebarVisibilityState;
  onEnabledToggle?: () => void;
  dragHandleProps?: ButtonHTMLAttributes<HTMLButtonElement>;
  dragHandleRef?: (element: HTMLButtonElement | null) => void;
}

export function SidebarAccordion({
  value,
  onValueChange,
  children,
  level = 'pane',
}: SidebarAccordionProps) {
  return (
    <Accordion.Root
      type="multiple"
      value={value}
      onValueChange={onValueChange}
      className={`prototype-accordion prototype-accordion--${level}`}
    >
      {children}
    </Accordion.Root>
  );
}

export function SidebarChevronDownIcon({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden="true"
      className={className}
    >
      <path d="m6 9 6 6 6-6" />
    </svg>
  );
}

export function SidebarAccordionItem({
  id,
  title,
  subtitle,
  badge,
  children,
  level = 'pane',
  panel = false,
  enabled,
  toggleState,
  onEnabledToggle,
  dragHandleProps,
  dragHandleRef,
}: SidebarAccordionItemProps) {
  return (
    <Accordion.Item
      value={id}
      className={`prototype-accordion-item prototype-accordion-item--${level}${
        panel ? ' prototype-accordion-item--panel' : ''
      }`}
    >
      <Accordion.Header className="prototype-accordion-item__heading">
        <div className="prototype-accordion-item__header-bar">
          <span className="prototype-accordion-item__title-wrap">
            <span className="prototype-accordion-item__title">{title}</span>
            {subtitle ? (
              <span className="prototype-accordion-item__subtitle">{subtitle}</span>
            ) : null}
          </span>
          {level === 'pane' ? (
            <>
              <span className="prototype-accordion-item__pane-main-slot">
                {typeof enabled === 'boolean' ? (
                  <ExactToggleButton
                    enabled={enabled}
                    state={toggleState}
                    onClick={onEnabledToggle}
                  />
                ) : badge ? (
                  <ExactMetricPill summary={badge} />
                ) : null}
              </span>
              <Accordion.Trigger className="prototype-disclosure-button prototype-disclosure-button--pane">
                <SidebarChevronDownIcon className="prototype-accordion-item__chevron" />
              </Accordion.Trigger>
              <ExactDragHandle
                ref={dragHandleRef}
                className="prototype-drag-handle--pane"
                label={title}
                {...dragHandleProps}
              />
            </>
          ) : (
            <span className="prototype-accordion-item__meta">
              {typeof enabled === 'boolean' ? (
                <ExactToggleButton
                  enabled={enabled}
                  state={toggleState}
                  onClick={onEnabledToggle}
                />
              ) : null}
              {badge ? <ExactMetricPill summary={badge} /> : null}
              <Accordion.Trigger className="prototype-disclosure-button">
                <SidebarChevronDownIcon className="prototype-accordion-item__chevron" />
              </Accordion.Trigger>
            </span>
          )}
        </div>
      </Accordion.Header>
      <Accordion.Content className="prototype-accordion-item__content">
        {children}
      </Accordion.Content>
    </Accordion.Item>
  );
}
